...
@section('content')
    ...
    <section aria-labelledby="products-heading" class="pb-24 pt-6">
        ...
        <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
            <!-- Filters -->
            <form action="{{ route('home') }}" method="get" enctype="multipart/form-data" class="hidden lg:block">
                ...
                <div class="border-b border-gray-200 py-6">
                    ...
                    <div class="pt-6 hidden" id="filter-section-1">
                        <div class="space-y-4">
                            @foreach($categories as $category)
                                <div class="flex items-center">
                                    <input id="filter-category-{{ $category->id }}" name="category[]" value="{{ $category->id }}" type="checkbox" @if(request()->has('category') && in_array($category->id, request('category'))) checked @endif class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                    <label for="filter-category-{{ $category->id }}" class="ml-3 text-sm text-gray-600">{{ $category->title }}</label>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    @foreach($products as $product)
                        <div class="group flex flex-col flex-wrap justify-between p-6 border border-gray-200 rounded-lg hover:-translate-y-1 transform-gpu transition ease-in-out">
                            <div class="aspect-h-1 aspect-w-1 h-1/2 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7">
                                    ...
                                    <div class="swiper-wrapper">
                                        @foreach($product->img as $img)
                                            <div class="swiper-slide">
                                                <img id="product_image" src="{{ asset('storage/' . $img) }}" class="h-56 w-full object-cover object-center group-hover:opacity-75" alt="image_item">
                                            </div>
                                        @endforeach
                                    </div>
                                ...
                                <script>
                                    var swiper = new Swiper(".mySwiper-{{$product->id}}", {
                                        autoplay: {
                                            delay: 2500,
